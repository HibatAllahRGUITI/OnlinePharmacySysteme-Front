...
import { updateOrdonnanceStatus, getOrdonnanceEnCours } from '../services/pharmacyApi';
...
export default function TableOrdonnPharmaEnCours() {
  ...
  const statuses = ['EN_COURS', 'TRAITEE', 'ANNULEE'];
  ...
  const handleStatusChange =async  (id) => {
    ...
    const fetchRequests = async () => {
      try {
        const data = await getOrdonnanceEnCours();
        if (!data || data.error) {
...
const handleOpen = async (id) => {
  ...
  return (
    ...
    >
      Ordonnances en cours de traitement ⌛
    </h2>
