...
import {  getOrdonnanceRejected } from '../services/pharmacyApi';
...
const columns = [
    ...
    { id: "dateRejection", label: "Date de refus", minWidth: 200}
];

export default function TableOrdonnPharmaciAnnule() {
  ...
  useEffect(() => {
    async function fetchRequests() {
        try {
            const data = await getOrdonnanceRejected();
            if (!data || data.error) {
            ...
            <TableBody>
              { ordonnances.length === 0 ? (
                ...
                <TableRow hover key={ordonnance.id} onClick={() => handleRowClick(ordonnance)}>
                  ...
                  <TableCell>{new Date(ordonnance.dateRejection).toLocaleDateString("fr-FR")}</TableCell>
                  <TableCell>
